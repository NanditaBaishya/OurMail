<?php
  ...
  // in data control
  if($isVal == 'true'){
    echo 'The email id and user name must be unique for each and every user';
    die();
  };
  
  if(!$fullname || !$email || !$username || !$password){
    echo 'During registration, no field must be left empty';
  }else{
    $dat = [];
    ...
    if($execute){
      ...
      echo 'The user registration is successful!';
    }else{
      echo 'The user registration is not successful!';
    };
